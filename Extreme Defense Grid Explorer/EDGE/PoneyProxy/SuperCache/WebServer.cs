namespace PonyProxy.SuperCache
{
    ...
    internal sealed class WebServer : IDisposable
    {
        ...
        public WebServer()
        {
            PreloadScripts = new List<PreloadScript>();
        }
...
#else
                return SendingRequestRegistrationTokenTable.AddEventHandler(value);
#endif
            }

            remove
            {
                SendingRequestRegistrationTokenTable.RemoveEventHandler(value);
            }
        }
...
#else
                return TextResponseReceivedRegistrationTokenTable.AddEventHandler(value);
#endif
            }

            remove
            {
                TextResponseReceivedRegistrationTokenTable.RemoveEventHandler(value);
            }
        }
...
#else
                return OfflinePageUnavailableRegistrationTokenTable.AddEventHandler(value);
#endif
            }

            remove
            {
                OfflinePageUnavailableRegistrationTokenTable.RemoveEventHandler(value);
            }
        }
        ...
        internal EventRegistrationTokenTable<EventHandler<SendingRequestEventArgs>> SendingRequestRegistrationTokenTable
        {
            get
            {
                return EventRegistrationTokenTable<EventHandler<SendingRequestEventArgs>>
                    .GetOrCreateEventRegistrationTokenTable(ref sendingRequestRegistrationTokenTable);
            }
        }
        ...
        internal EventRegistrationTokenTable<EventHandler<ResponseReceivedEventArgs>> TextResponseReceivedRegistrationTokenTable
        {
            get
            {
                return EventRegistrationTokenTable<EventHandler<ResponseReceivedEventArgs>>
                    .GetOrCreateEventRegistrationTokenTable(ref textResponseReceivedRegistrationTokenTable);
            }
        }
        ...
        internal EventRegistrationTokenTable<EventHandler<OfflinePageUnavailableEventArgs>> OfflinePageUnavailableRegistrationTokenTable
        {
            get
            {
                return EventRegistrationTokenTable<EventHandler<OfflinePageUnavailableEventArgs>>
                    .GetOrCreateEventRegistrationTokenTable(ref offlinePageUnavailableRegistrationTokenTable);
            }
        }
        ...
        public static string BuildProxyUri(Uri proxyUri, Uri baseUri, string requestUri, IEnumerable<string> whiteList)
        {
            Uri targetUri;
            ...
            if (Uri.IsWellFormedUriString(requestUri, UriKind.RelativeOrAbsolute))
            {
                ...
                if (!(string.Equals(targetHost.Scheme, "http", StringComparison.OrdinalIgnoreCase) || string.Equals(targetHost.Scheme, "https", StringComparison.OrdinalIgnoreCase)))
                {
                    return requestUri;
                }
        ...
        {
            return CleanupAsync().AsAsyncAction();
        }

        public string BuildCurrentProxyUri(Uri baseUri, string requestUri)
        {
            return BuildProxyUri(proxyUri, baseUri, requestUri, configuration.WhiteList);
        }
        ...
        {
            return ResolveTargetUri(baseUri, requestUri);
        }
        ...
        {
            if (configuration.Mode == DefenseMode.PoneyAugmentedProtectionAnalyzer)
            {
                var dynamicImageHandler = await ReadScriptAsync("/PonyProxy/SuperCache/Scripts/DynamicImageHandler.html");
                PreloadScripts.Add(new PreloadScript(dynamicImageHandler, -1));
            }
        }
        ...
        {
            Dispose(true);
            await cookieManager.PersistCookiesAsync(activityId);
            Trace.Information(activityId, "Server has stopped.");
        }
        ...
        private async Task OnConnectionReceivedAsync(StreamSocketListener sender, StreamSocketListenerConnectionReceivedEventArgs args)
        {
            ...
            using (var socket = args.Socket)
            {
                ...
                using (var request = await ReadRequestAsync(socket, requestId))
                {
                    if (request != null)
                    {
                        using (var response = await ForwardRequestAsync(request, requestId))
                        {
                            ...
                            if (response != null && (response.StatusCode == HttpStatusCode.Redirect || response.StatusCode == HttpStatusCode.Found))
                            {
                                ...
                                response.Headers.Location = new Uri(BuildCurrentProxyUri(response.Headers.Location, response.Headers.Location.OriginalString));
                            }

                            var setCookieHeaders = cookieManager.ProcessResponseCookies(requestId, response);
                            await WriteResponseAsync(socket, response, requestId, setCookieHeaders);

                            if (response == null)
                            {
                                var handler = OfflinePageUnavailableRegistrationTokenTable.InvocationList;
                                if (handler != null)
                                {
                                    if (context != null)
                                    {
        ...
        private async Task<HttpResponseMessage> ForwardRequestAsync(HttpRequestMessage request, Guid requestId)
        {
            ...
            await OnSendingRequestAsync(request, requestId);
            var response = await requestResolver.ResolveRequestAsync(request, requestId);
            if (response != null)
            {
            ...
            await OnResponseReceivedAsync(response, requestId);
        ...
        {
            var sendingRequest = SendingRequestRegistrationTokenTable.InvocationList;
            if (sendingRequest != null)
            {
                ...
                await RunThroughDispatcherAsync(() => sendingRequest(this, eventArgs));
            }
        }
        ...
        private async Task OnResponseReceivedAsync(HttpResponseMessage response, Guid requestId)
        {
            ...
            if (content != null && content.Headers.ContentType != null)
            {
                ...
                if (mediaType == "text" || (mediaType == "application" && (mediaSubType == "json" || mediaSubType == "xhtml+xml")))
                {
                    ...
                    var responseReceived = TextResponseReceivedRegistrationTokenTable.InvocationList;
                    if (responseReceived != null)
                    {
                        ...
                        await RunThroughDispatcherAsync(() => responseReceived(this, eventArgs));
                        htmlContent = eventArgs.Content;
                    }

                    if (configuration.Mode == DefenseMode.PoneyAugmentedProtectionAnalyzer)
                    {
                        htmlContent = htmlContent.Replace("background-image", "pony-image");
                    }
                    ...
                    if (mediaType == "text" || mediaType == "application")
                    {
                        ...
                        {
                            var processor = new HtmlProcessor(htmlContent, this, baseUri);
                            processor.RedirectLinks(response.RequestMessage.RequestUri);

                            if (configuration.WhiteList.Count(u => string.Compare(baseUri.Authority, u, StringComparison.CurrentCultureIgnoreCase) == 0) == 0)
                            {
                                if (configuration.Mode == DefenseMode.NoSlangAnalyzer || configuration.Mode == DefenseMode.PoneyAugmentedProtectionAnalyzer)
                                {
                                    await processor.PonyfyTextsAsync();
                                }

                                if (configuration.Mode == DefenseMode.PoneyAugmentedProtectionAnalyzer)
                                {
                                    processor.ChangeImages();
                                    processor.RemoveFlashAndObjects();
                                    processor.ChangeVideos();
                                }
                            }

                            // Inject preload scripts in HTML content
                            foreach (var preloadScript in PreloadScripts.OrderByDescending(p => p.Priority))
                            {
                        ...
                        else if (mediaSubType == "css")
                        {
                            htmlContent = Regex.Replace(
                                ...
                                (match) =>
                                {
                                    var rewriteUri = BuildCurrentProxyUri(response.RequestMessage.RequestUri, match.Groups["url"].Value);
